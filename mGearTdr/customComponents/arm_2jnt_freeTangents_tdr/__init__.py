...
class Component(component.Main):
    ...
    def addObjects(self):
        ...
        # Base for twist ref
        t = transform.getTransformLookingAt(pm.datatypes.Vector(0, 0, 0),
                                            pm.datatypes.Vector(1, 0, 0),
                                            self.normal,
                                            "xz",
                                            self.negate)
        self.base = primitive.addTransform(self.root, self.getName('base'), t)
        self.base.t.set(0, 0, 0)
        ...
        self.armRollRef = primitive.add2DChain(
            ...
            False,
            self.WIP)
        self.armRollRef[0].setParent(self.base)
        jo = self.armRollRef[0].jointOrient.get()
        self.armRollRef[0].jointOrient.set(0,0,0)
        self.armRollRef[0].r.set(jo)
    ...
    def addOperators(self):
        ...
        self.ikhArmRef, self.tmpCrv = applyop.splineIK(
            ...
            cParent=self.bone0)
        '''
        self.ikhArmRef = pm.ikHandle(n=self.getName('armRollRefIkHandle'),
                                    sj=self.armRollRef[0],
                                    ee=self.armRollRef[-1])[0]
        self.ikhArmRef.setParent(self.mid_ctl)
        self.ikhArmRef.poleVector.set(0,0,0)
        self.ikhArmRef.t.set(0, 0, 0)
